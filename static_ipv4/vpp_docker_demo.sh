...
# What is VPE
#vpp unix {cli-listen 0.0.0.0:5002 startup-config $VPP_CONF } dpdk {no-pci no-huge num-mbufs 8192}
...
ip netns exec "$CONTAINER_1_NS" iptables -A OUTPUT -p icmp -o $GLOBAL_INT -j REJECT
ip netns exec "$CONTAINER_2_NS" iptables -A OUTPUT -p icmp -o $GLOBAL_INT -j REJECT

# TODO: Add NAT to Linux Host
